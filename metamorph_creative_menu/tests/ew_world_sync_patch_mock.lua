...
local function verify(source, label)
    ...
    assert(string.find(patched, "mcm_poly_world_sync_v5", 1, true), label .. " marker missing")
    assert(string.find(patched, "mcm_boss_death_pressure_active", 1, true), label .. " death backpressure missing")
    assert(string.find(patched, "mcm_heavy_polymorph_active", 1, true), label .. " heavy-form backpressure missing")
    assert(string.find(patched, 'EntityGetFirstComponentIncludingDisabled(entity, "BossDragonComponent")', 1, true), label .. " BossDragon heavy-form detection missing")
    assert(string.find(patched, 'EntityGetFirstComponentIncludingDisabled(entity, "WormComponent")', 1, true), label .. " Worm heavy-form detection missing")
    assert(string.find(patched, "mcm_poly_scheduler_paused", 1, true), label .. " scheduler pause metric missing")
    assert(string.find(patched, "mcm_trail_queue", 1, true) == nil, label .. " legacy trail queue survived")
    assert(string.find(patched, "mcm_drain_trail", 1, true) == nil, label .. " legacy trail drain survived")
    assert(string.find(patched, "mcm_world_sync_trail_backlog_v1", 1, true), label .. " compatibility backlog metric missing")

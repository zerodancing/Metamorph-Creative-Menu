...
-- Name writes and apply verification must share the same UTF-8-safe 160-byte result.
local long_cjk_name=string.rep('漢',54)
local safe_cjk_name=string.rep('漢',53)
assert(appearance.set_name(1,2,long_cjk_name,true)==true,'UTF-8 set_name failed')
snap=assert(appearance.snapshot(2)); assert(snap.name==safe_cjk_name and #snap.name==159,'set_name split a UTF-8 codepoint')
assert(appearance.apply(1,2,{meta={name=long_cjk_name,show_name_in_ui=true}})==true,'UTF-8 appearance apply failed')
snap=assert(appearance.snapshot(2)); assert(snap.name==safe_cjk_name and #snap.name==159,'appearance apply wrote or verified a different UTF-8 name')
